...
Настроенное облако (шаг 9а) снимает этот вопрос: копия сразу уезжает в хранилище.

## 9а. Облачное хранилище (Yandex Object Storage)

Без него креативы и фото лежат на диске сервера (40 ГБ), а бэкапы — там же, где и
сама база. С ним ролики и фото уходят в облако, на диске не задерживаются, а копии
базы дублируются вне сервера.

**В консоли Yandex Cloud** (https://console.yandex.cloud):

1. Создайте бакет: **Object Storage → Создать бакет**. Имя, например `led-list`,
   доступ **закрытый** — файлы отдаются приложением по временным ссылкам, публичным
   бакет быть не должен.
2. **IAM → Сервисные аккаунты → Создать**, роль `storage.editor` на каталог.
3. У этого аккаунта: **Создать новый ключ → Статический ключ доступа**. Появятся
   идентификатор и секрет — секрет показывается один раз.

**На сервере** впишите их в `/etc/led-list/api.env`:
...
Перезапустите сервис и проверьте лог — там должна появиться строка про облако:

```bash
systemctl restart led-list-api && journalctl -u led-list-api -n 5 --no-pager | grep Облако
```

Проверить целиком проще всего бэкапом — он и создаст копию, и отправит её:

```bash
cd /var/www/led-list/server && node_modules/.bin/tsx scripts/backup.ts
```

Строка `Отправлено в облако: backups/led-<дата>.db` означает, что ключи рабочие.

Что важно знать:

- **Уже загруженные файлы остаются на диске.** У каждой записи в БД помечено, где
  лежит её файл, и старые продолжают отдаваться с диска — в облако уезжает только
  то, что загрузили после настройки.
- **Если облако недоступно**, загрузка не срывается: файл остаётся на диске, в логе
  появляется причина. Ролик не теряется.
- Файлы отдаются по временным ссылкам (10 минут), поэтому бакет держите закрытым.
- Оплата — по объёму и запросам, порядка 2 ₽ за ГБ в месяц.
