...
Администратор системы создаётся с паролем из README (`info@novayaera.com` / `123456789`) —
годится, пока адрес известен только вам, но на публичном сервере такой пароль подбирается
за секунды. Смените его сразу после первого входа: **Настройки → Мой аккаунт** —
там же меняются имя и email-логин.
